...
action.add_parser("hide", help="generate thundra.toml from thundra-dev.toml file to hide secrets value")
install = action.add_parser("install")
install.add_argument('--no-generate', action='store_true', default=False,help='don\'t generate thundra-dev.toml')
    ...
    match parse.action:
        ...
        case "install":
            ...
            if not parse.no_generate:
                open(workdir.config_path.parent / "thundra-dev.toml", "w").write(tomli_w.dumps(config_toml))
