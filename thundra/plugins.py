...
class PluginZip:
    ...
    def get_info(self) -> dict:
        ...
        return tomllib.loads(self.file.open(root + 'thundra-plugin.toml').read().decode())
    ...
    def install(self):
        ...
        config_toml['plugins'][name] = {'username': self.username, 'repository': self.repos, 'branch': self.branch}
...
class PluginSource(requests.Session):
    ...
    def latest_version(self) -> dict:
        req = self.get(f'https://api.github.com/repos/{self.git_username}/{self.repository}/releases')
        if req.status_code == 200:
            return req.json()[0]
        raise Response404()
    
    def download_release(self, version: str) -> PluginZip:
        req = requests.get(f'https://github.com/{self.git_username}/{self.repository}/archive/refs/tags/{version}.zip')
        if req.status_code == 200:
            return PluginZip(BytesIO(req.content), self.git_username, self.repository, version)
        raise Response404(version)
    
    def download_head(self, branch: str) -> PluginZip:
        req = requests.get(f'https://github.com/{self.git_username}/{self.repository}/archive/refs/heads/{branch}.zip')
        if req.status_code == 200:
